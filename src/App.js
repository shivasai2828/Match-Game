...
const FruitsItems = imagesList.filter(eachList => eachList.category === 'FRUIT')
...
class App extends Component {
  state = {
    ...
    seconds: 60,
    score: 0,
    listItems: FruitsItems,
    category: 'FRUIT',
    mainImgUrl: imagesList[0].imageUrl,
  }
  componentDidMount() {
    this.timerId = setInterval(this.tick, 1000)
  }
  playagainBtn = () => {
    this.setState(preve => ({isEnd: !preve.isEnd, seconds: 60, score: 0}))
  }
  tick = () => {
    ...
    if (seconds !== 0) {
      ...
      this.setState(prev => ({isEnd: !prev.isEnd}))
    }
  }

  checkIsMatchedOr = thumbnailUrl => {
    console.log(thumbnailUrl)
    const {mainImgUrl} = this.state
    const clickedimgDetails = imagesList.filter(
      eachList => eachList.thumbnailUrl === thumbnailUrl,
    )
    console.log(clickedimgDetails)
    const {imageUrl} = clickedimgDetails[0]
    if (mainImgUrl === imageUrl) {
      console.log('its correcr')
      const newimg =
  ...
  changeTab = id => {
    ...
    const updateList = imagesList.filter(eachList => eachList.category === id)
    this.setState({listItems: updateList})
  }
  render() {
    const {mainImgUrl, listItems, category, isEnd, showTab, seconds, score} =
      this.state
          ...
          <ul className="unorder-list-nav">
            <li>
              <p className="score-text">
                Score: <span className="color-1">{score}</span>
              </p>
            </li>
            <li className="row-list-item">
              <img
                ...
                alt="timer"
              />
              <p className="color-1">{seconds} sec</p>
            </li>
          </ul>
        </nav>
        {isEnd ? (
          <div className="win-loss-container">
            <div className="botttom">
              ...
              <button onClick={this.playagainBtn} className="play-again-btn">
                <img
                  className="reset-img"
          ...
          <div className="bg-main">
            <div className="tab-container">
              <div className="main-img-container">
                <img className="main-img" src={mainImgUrl} alt="match" />
              </div>
            ...
            <div>
              <ul className="img-lists">
                {listItems.map(eachList => (
                  <TabListItems
                    key={eachList.id}
                    checkIsMatchedOr={this.checkIsMatchedOr}
                    eachList={eachList}
                  />
                ))}
              </ul>
